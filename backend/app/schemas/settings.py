...
class UserSettings(BaseModel):
    ...
    def validate_primary_language(cls, v):
        ...
        if v not in supported_languages:
            raise ValueError(f"Primary language '{v}' is not supported. Supported languages: {', '.join(supported_languages)}")
        return v
    ...
    def validate_target_language(cls, v):
        supported_languages = list(LANGUAGE_CONFIG.keys())
        if v not in supported_languages:
            raise ValueError(f"Target language '{v}' is not supported. Supported languages: {', '.join(supported_languages)}")
        return v
