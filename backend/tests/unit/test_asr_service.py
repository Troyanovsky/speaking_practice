...
async def test_transcribe_win_transcription_key(asr_service):
    with patch("sys.platform", "win32"):
        ...
        mock_model.transcribe.return_value = [{"transcription": "Hello world transcription"}]
        asr_service.model = mock_model
...
async def test_transcribe_win_tuple_of_lists(asr_service):
    with patch("sys.platform", "win32"):
        ...
        mock_model.transcribe.return_value = (["Hello world tuple list"], ["Hello world tuple list"])
        asr_service.model = mock_model
